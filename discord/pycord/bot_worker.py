...
# Pycord port of bot_worker.py (A2 PR2 Phase 2d).
# Migration delta vs ../bot_worker.py:
#   * `discord.Client` + `app_commands.CommandTree(client)` → `discord.Bot(...)`.
#     Bot subclasses Client; `client` alias preserved.
#   * `@client.slash_command(...)` → `@client.slash_command(...)`.
#   * Slash callbacks: `ctx: discord.ApplicationContext` → `ctx: discord.ApplicationContext`,
#     and `interaction.X` → `ctx.X` (channel, channel_id, guild, user, followup),
#     `ctx.respond(...)` → `ctx.respond(...)`,
#     `ctx.defer(...)` → `ctx.defer(...)`.
#   * `app_commands.choices(...)` decorators → `discord.OptionChoice` inside `discord.Option`.
#   * `app_commands.describe(...)` → `discord.Option(description=...)` per parameter.
#   * Slash sync: pycord auto-syncs when `Bot(debug_guilds=[...])` is set; no manual tree.sync.
#   * View imports unchanged — Views use `discord.Interaction` natively in both libs.
#   * on_message, _http, helper functions, voice flow, state dicts: all unchanged.
import discord
import httpx
...
client = discord.Bot(**_bot_kwargs)  # Bot subclasses Client; legacy `client` alias preserved
...
@client.slash_command(name="new", description="Start a fresh conversation in a new channel")
async def cmd_new(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return

    await ctx.defer(ephemeral=True)   # channel creation takes > 3s
    ...
    await new_channel.send(
        f"Hey {ctx.user.mention}! New conversation started. What's on your mind?"
    )
...
@client.slash_command(name="clear", description="Reset conversation and delete all messages in this channel")
async def cmd_clear(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return

    await ctx.defer(ephemeral=True)   # purge may take a moment
    ...
    reset_channel_session(channel.id, ctx.user.id)
    ...
    try:
        ...
        await ctx.followup.send(f"Could not delete messages: {e}", ephemeral=True)
        return
...
@client.slash_command(name="mode", description="Show or switch agent mode")
async def cmd_mode(
    ctx: discord.ApplicationContext,
    mode: discord.Option(
        str,
        description="Leave blank to show current; otherwise: plan, build, or converse",
        choices=[
            discord.OptionChoice(name="plan",     value="plan"),
            discord.OptionChoice(name="build",    value="build"),
            discord.OptionChoice(name="converse", value="converse"),
        ],
        required=False,
        default=None,
    ) = None,
    ...
    if not is_allowed(ctx.user.id):
        ...
        return
    if mode is None:
        current = _get_mode(ctx.channel_id, ctx.user.id)
        await ctx.respond(
            f"**Current mode**: `{current}`. {_MODE_DESCRIPTIONS.get(current, '')}"
        )
        return
    # Pycord passes the chosen value directly (a str), not a Choice wrapper.
    _apply_mode(ctx.channel_id, ctx.user.id, mode)
    await ctx.respond(
        f"Switched to **{mode}** mode. {_MODE_DESCRIPTIONS[mode]}"
    )


@client.slash_command(name="plan", description="Switch to plan mode (investigate + create plan)")
async def cmd_plan(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    _apply_mode(ctx.channel_id, ctx.user.id, "plan")
    await ctx.respond(
        f"Switched to **plan** mode. {_MODE_DESCRIPTIONS['plan']}"
    )


@client.slash_command(name="build", description="Switch to build mode (execute active plan)")
async def cmd_build(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    _apply_mode(ctx.channel_id, ctx.user.id, "build")
    await ctx.respond(
        f"Switched to **build** mode. {_MODE_DESCRIPTIONS['build']}"
    )


@client.slash_command(name="converse", description="Switch to converse mode (quick chat)")
async def cmd_converse(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    _apply_mode(ctx.channel_id, ctx.user.id, "converse")
    await ctx.respond(
...
async def _model_autocomplete(actx: discord.AutocompleteContext) -> list[discord.OptionChoice]:
    """Pycord autocomplete: receives AutocompleteContext (.value = user's typed text).
    Returns list of OptionChoice (name shown to user, value sent on select)."""
    presets = await _fetch_model_presets()
    cur = (actx.value or "").lower()
    items: list[discord.OptionChoice] = []
    for name, spec in presets.items():
        ...
        items.append(discord.OptionChoice(name=label[:100], value=name))
        if len(items) >= 25:  # Discord caps autocomplete at 25 entries
            break
    return items


@client.slash_command(name="model", description="Show or switch the current LLM model")
async def cmd_model(
    ctx: discord.ApplicationContext,
    preset: discord.Option(
        str,
        description="Switch to a named model from config.models (leave blank to show current)",
        autocomplete=_model_autocomplete,
        required=False,
        default=None,
    ) = None,
):
...
@client.slash_command(name="status", description="Show current session mode, model, and system status")
async def cmd_status(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    mode = _get_mode(ctx.channel_id, ctx.user.id)
    # Show channel session if applicable, otherwise user+channel session
    if ctx.channel_id in _channel_sessions:
        session_id = _channel_sessions[ctx.channel_id]
    else:
        session_id = _get_session_id(ctx.channel_id, ctx.user.id)
    try:
        resp = await _http.get(f"{PHOEBE_API_URL}/config")
        resp.raise_for_status()
        cfg = resp.json()
...
@client.slash_command(name="context", description="Show context-window stats for this channel's session")
async def cmd_context(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    if ctx.channel_id in _channel_sessions:
...
@client.slash_command(name="btw", description="Add context to the current run without stopping it")
async def cmd_btw(
    ctx: discord.ApplicationContext,
    text: discord.Option(str, description="The note to pass to the agent (stapled onto the next tool result)"),
):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    sid = _channel_in_flight.get(ctx.channel_id)
    if not sid:
        await ctx.respond("No active run in this channel.", ephemeral=True)
        return
    result = await _post_injection(sid, text, mode="not_urgent")
    if result is None:
        await ctx.respond("Injection failed.", ephemeral=True)
        return
    snippet = text if len(text) <= 80 else text[:80] + "…"
    await ctx.respond(f"-# \U0001f4dd injected: {snippet}")
...
@client.slash_command(name="stop", description="Stop the current worker run in this channel")
async def cmd_stop(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    sid = _channel_in_flight.get(ctx.channel_id)
    ...
    now = time.time()
    last_stop = _stop_request_ts.get(ctx.channel_id)
    if last_stop is not None and now - last_stop < _STOP_HARD_CANCEL_WINDOW_S:
        ...
        try:
            ...
            await ctx.respond(f"Hard cancel failed: {e}", ephemeral=True)
        finally:
            _stop_request_ts.pop(ctx.channel_id, None)
        return

    result = await _post_injection(sid, "", mode="stop")
    if result is None:
        await ctx.respond("Stop request failed.", ephemeral=True)
        return
    _stop_request_ts[ctx.channel_id] = now
    await ctx.respond(
...
@client.slash_command(name="help", description="List available slash commands")
...
def _session_for_channel(ctx: discord.ApplicationContext) -> str:
    """Return the channel's session id, creating a per-user one as fallback."""
    if ctx.channel_id in _channel_sessions:
        return _channel_sessions[ctx.channel_id]
    return _get_session_id(ctx.channel_id, ctx.user.id)


@client.slash_command(name="diag", description="Run system diagnostic checks and show overall health")
async def cmd_diag(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    await ctx.defer()
    try:
        ...
        await ctx.followup.send(f"Diagnostics unavailable: {e}", ephemeral=True)
        return
...
@client.slash_command(name="memory", description="Search stored memories (ChromaDB)")
async def cmd_memory(
    ctx: discord.ApplicationContext,
    query: discord.Option(str, description="What to search for"),
    n: discord.Option(int, description="How many results (default 5)", required=False, default=5) = 5,
):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    ...
    try:
        ...
        await ctx.followup.send(f"Memory search failed: {e}", ephemeral=True)
        return

    results = data.get("results") or []
    if not results:
        await ctx.followup.send(f"No memories matched `{query}`.")
        return
...
@client.slash_command(name="todos", description="Show todos and checkpoints for this session")
async def cmd_todos(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    sid = _session_for_channel(ctx)
    try:
        resp = await _http.get(f"{PHOEBE_API_URL}/v1/sessions/{sid}/state", timeout=5)
        resp.raise_for_status()
        state = resp.json()
    except Exception as e:
        await ctx.respond(f"Could not read state: {e}", ephemeral=True)
        return
...
@client.slash_command(name="kill", description="Hard-cancel the in-flight worker run immediately")
async def cmd_kill(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    sid = _channel_in_flight.get(ctx.channel_id)
    ...
    if resp.status_code != 200:
        ...
        return
    _stop_request_ts.pop(ctx.channel_id, None)
    await ctx.respond("🛑 **Hard cancel** — worker terminated.")


@client.slash_command(name="retry", description="Re-run the last user message in this session")
async def cmd_retry(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    if _channel_in_flight.get(ctx.channel_id):
        await ctx.respond(
    ...
    try:
        ...
        await ctx.respond(f"Could not read session: {e}", ephemeral=True)
        return
    if resp.status_code == 404:
        await ctx.respond("No history in this session yet.", ephemeral=True)
        return
    if resp.status_code != 200:
        await ctx.respond(
    ...
    if not last_user.strip():
        await ctx.respond("Nothing to retry.", ephemeral=True)
        return

    snippet = last_user if len(last_user) <= 80 else last_user[:80] + "…"
    await ctx.respond(f"🔁 Retrying: {snippet}")
    mode = _get_mode(ctx.channel_id, ctx.user.id)
    asyncio.create_task(_execute_retry(sid, ctx.channel, last_user, mode))


async def _revoke_autocomplete(actx: discord.AutocompleteContext) -> list[str]:
    """Pycord autocomplete returning the session's always-allow list filtered by typed prefix."""
    # AutocompleteContext exposes the parent interaction's channel via .interaction.
    sid_ctx = actx.interaction
    sid = (_channel_sessions.get(sid_ctx.channel_id)
           or _get_session_id(sid_ctx.channel_id, sid_ctx.user.id))
    approved = sorted(_session_always_allow.get(sid, set()))
    cur = (actx.value or "").lower()
    return [t for t in approved if not cur or cur in t.lower()][:25]


@client.slash_command(name="revoke", description="Revoke a session-approved tool (next use re-prompts)")
async def cmd_revoke(
    ctx: discord.ApplicationContext,
    tool: discord.Option(
        str,
        description="The tool name to revoke from this session",
        autocomplete=_revoke_autocomplete,
    ),
):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
...
@client.slash_command(name="resume", description="Bind this channel to a different session id")
async def cmd_resume(
    ctx: discord.ApplicationContext,
    session_id: discord.Option(str, description="The session id to resume (see /sessions via api for a list)"),
):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    try:
        ...
        await ctx.respond(f"Could not fetch session: {e}", ephemeral=True)
        return
    ...
    _channel_sessions[ctx.channel_id] = session_id
    WORKER_CHANNEL_IDS.add(ctx.channel_id)
    _save_state()
    await ctx.respond(
...
@client.slash_command(name="compact", description="Force-run the rolling compactor for this session")
async def cmd_compact(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    sid = _session_for_channel(ctx)
    ...
    try:
        ...
        await ctx.followup.send(f"Compaction failed: {e}", ephemeral=True)
        return

    if "skipped" in result:
        await ctx.followup.send(f"Skipped: `{result['skipped']}`.")
        return
    if "error" in result:
        await ctx.followup.send(f"❌ Compaction error: `{result['error']}`.")
        return
...
@client.slash_command(name="dream-run", description="Manually trigger a verbose dream run with per-edit review")
async def cmd_dream_run(
    ctx: discord.ApplicationContext,
    date: discord.Option(str, description="UTC calendar day YYYY-MM-DD (default: rolling last 24h)", required=False, default=None) = None,
    window_hours: discord.Option(float, description="Rolling-window size in hours (default 24; ignored when date is set)", required=False, default=None) = None,
    meta: discord.Option(bool, description="Run the meta-dreamer after per-conversation passes (default: true)", required=False, default=True) = True,
    review: discord.Option(bool, description="Gate finalize on your accept/drop decisions (default: true)", required=False, default=True) = True,
):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
...
@client.slash_command(name="soul", description="Show the current SOUL.md contents")
async def cmd_soul(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    await ctx.defer()
    try:
        ...
        await ctx.followup.send(f"Could not read SOUL: {e}", ephemeral=True)
        return

    content = (data.get("content") or "").strip()
    if not content:
        await ctx.followup.send("SOUL.md is empty or missing.")
        return

    chunks = split_message(content)
    await ctx.followup.send(f"**SOUL.md** ({len(content):,} chars)")
    for chunk in chunks:
        await ctx.channel.send(chunk)


@client.slash_command(name="plan-show", description="Show this session's active plan")
async def cmd_plan_show(ctx: discord.ApplicationContext):
    if not is_allowed(ctx.user.id):
        await ctx.respond("Not authorized.", ephemeral=True)
        return
    sid = _session_for_channel(ctx)
    ...
    plan = (plan or "").strip()
    if not plan:
        await ctx.respond("No active plan for this session.")
        return

    chunks = split_message(plan)
    await ctx.respond(f"**Active plan** — {len(plan):,} chars")
    for chunk in chunks:
        await ctx.channel.send(chunk)


@client.slash_command(name="speak", description="Toggle auto-dictate for this channel — every reply is spoken in your VC while ON")
async def cmd_speak(ctx: discord.ApplicationContext):
...
async def on_ready():
    ...
    if GUILD_ID:
        ...
        # sync_commands ourselves or all 24 slashes silently disappear.
        try:
            synced = await client.sync_commands()
            n = len(synced) if synced else len(client.pending_application_commands)
            print(f"[worker-bot] synced {n} slash commands", flush=True)
        except Exception as e:
            print(f"[worker-bot] slash sync failed: {e}", flush=True)
...
async def on_message(msg: discord.Message):
    ...
    try:
        ...
        if not is_plan_output and _session_auto_dictate.get(session_id, False):
            ...
            if voice_ch:
                _voice_bot = os.environ.get("DISCORD_VOICE_BOT", "worker").strip() or "worker"
                async def _auto_dictate(text: str, vc_id: int) -> None:
                    try:
                        r = await _http.post(
                            "http://localhost:4000/discord/speak_voice",
                            json={"voice_channel_id": vc_id, "text": text,
                                  "block": True, "bot": _voice_bot},
                            timeout=180,
                        )
                        r.raise_for_status()
                    except Exception as e:
