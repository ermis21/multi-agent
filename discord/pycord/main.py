"""
Discord service — FastAPI HTTP API (port 4000) + both bots — Pycord variant
(A2 PR2 Phase 2e). Mirror of ../main.py with no Discord-library divergence
required: this file uses no `app_commands` and no `discord.Interaction`. The
`bot_config`, `bot_mod`, `bot_worker`, `views`, and `tts_backend` imports
resolve to the Pycord ports under discord/pycord/ via the docker-compose
mount layout.
...
# Discord audio worker calls source.read() every 20ms expecting one frame
# (3840 bytes = 20ms × 48kHz × 2ch × 2bytes s16le). Pull frames from a queue
# the synthesizer pushes into from a background thread. Bot starts speaking
# as soon as the first sentence is synthesized — no batch wait.

import queue as _queue
import threading as _threading

_FRAME_BYTES = 3840  # 20ms s16le 48kHz stereo


class StreamingPCMAudio(discord.AudioSource):
    """Pulls 20ms PCM frames from a queue. is_opus()=False so discord.py
    encodes the raw PCM to opus on the way out."""

    def __init__(self, max_buffer_frames: int = 500):  # ~10s
        self._q: _queue.Queue[bytes | None] = _queue.Queue(maxsize=max_buffer_frames)
        self._closed = False
        self._tail = b""

    def push_pcm(self, pcm: bytes) -> None:
        """Slice incoming PCM into 20ms frames + enqueue. Carries leftover
        bytes across calls so frame boundaries align even when chunks aren't
        multiples of 3840."""
        buf = self._tail + pcm
        n_full = len(buf) // _FRAME_BYTES
        for i in range(n_full):
            frame = buf[i * _FRAME_BYTES:(i + 1) * _FRAME_BYTES]
            self._q.put(frame)
        self._tail = buf[n_full * _FRAME_BYTES:]

    def signal_done(self) -> None:
        """Push leftover tail (zero-padded to one frame) + sentinel."""
        if self._tail:
            frame = self._tail + b"\x00" * (_FRAME_BYTES - len(self._tail))
            self._q.put(frame)
            self._tail = b""
        self._q.put(None)  # sentinel signals end-of-stream
    ...
    def read(self) -> bytes:
        ...
        try:
            ...
            # No data yet — return silence to keep the source alive.
...
def _stream_synthesis_to_source(text: str, backend_name: str,
                                 source: StreamingPCMAudio) -> None:
    """Background-thread synthesizer: yields per-sentence chunks from the
    backend, resamples each to 48kHz stereo s16le, pushes into the source's
    queue. audioop.ratecv state carries across calls so resampling is
    seamless. Marks source done when the iterator exhausts (or on error)."""
    import audioop as _ao
    try:
        backend = get_backend(backend_name)
        ratecv_state = None
        for samples_int16, sr in backend.synthesize(text):
            raw = samples_int16.tobytes()  # mono s16le at sr
            if sr != 48000:
                raw, ratecv_state = _ao.ratecv(raw, 2, 1, sr, 48000, ratecv_state)
            raw = _ao.tostereo(raw, 2, 1, 1)
            source.push_pcm(raw)
    except Exception as e:
        print(f"[voice/stream] synth error: {e}", flush=True)
    finally:
        source.signal_done()
...
async def _start_voice_playback(
    ...
    """Synthesize + LIVE-stream into a voice channel.

    block=True (auto-dictate / legacy /speak): wait for playback to finish.
    block=False (listen button): return as soon as playback begins so the
    view can swap to ⏸ Pause immediately.

    Either way the synthesis runs sentence-by-sentence in a background
    thread — bot starts speaking on the first sentence (~500ms after
    click), not after the entire text is synthesized."""
    ...
    # Diagnostics — what perms does the bot actually have in this VC?
    # Insufficient Connect / Speak is the #1 cause of "voice handshake
    # did not complete": Discord accepts the WebSocket but never sends
    # SESSION_DESCRIPTION, so vc.is_connected() never goes True.
    try:
        me = voice_channel.guild.me
        perms = voice_channel.permissions_for(me)
        print(
            f"[voice/diag] vc={voice_channel.name!r} ({voice_channel_id}) "
            f"bot={me} perms: connect={perms.connect} speak={perms.speak} "
            f"view_channel={perms.view_channel} use_voice_activation={perms.use_voice_activation} "
            f"existing_vcs_in_bot={len(getattr(bot_client, 'voice_clients', []) or [])}",
            flush=True,
        )
        if not perms.connect:
            return {"ok": False, "error": f"bot lacks Connect permission in #{voice_channel.name}"}
        if not perms.speak:
            return {"ok": False, "error": f"bot lacks Speak permission in #{voice_channel.name}"}
    except Exception as e:
        print(f"[voice/diag] permission probe failed: {e}", flush=True)

    backend_name = await _resolve_backend_name()
    loop = asyncio.get_event_loop()

    # Drain EVERY voice connection the bot still holds in this guild, not
    # just the one we tracked in _active_playback. A previous session can
    # leave a stale VoiceClient on the bot; reconnecting without disposing
    # it gives WebSocket 4006 ("session no longer valid"). Pycord exposes
    # bot.voice_clients as the authoritative list.
    target_guild_id = voice_channel.guild.id
    for stale_vc in list(getattr(bot_client, "voice_clients", []) or []):
        if getattr(stale_vc, "guild", None) and stale_vc.guild.id == target_guild_id:
            try:
                if stale_vc.is_playing() or stale_vc.is_paused():
                    stale_vc.stop()
            except Exception:
                pass
            try:
                await stale_vc.disconnect(force=True)
                print(f"[voice] drained stale vc in guild {target_guild_id}", flush=True)
            except Exception as e:
                print(f"[voice] stale-vc disconnect warn: {e}", flush=True)
    _active_playback.pop(voice_channel_id, None)

    # Brief settle after disconnect — Discord's voice gateway needs ~200ms
    # to invalidate the prior session before accepting a new connect with
    # the same bot+guild combo.
    await asyncio.sleep(0.3)

    print(f"[voice/diag] calling voice_channel.connect()…", flush=True)
    t0 = loop.time()
    try:
        vc = await voice_channel.connect(timeout=15.0, reconnect=True)
    except discord.ClientException as e:
        print(f"[voice] connect retry after ClientException: {e}", flush=True)
        for stale_vc in list(getattr(bot_client, "voice_clients", []) or []):
            try:
                await stale_vc.disconnect(force=True)
            except Exception:
        ...
        vc = await voice_channel.connect(timeout=15.0, reconnect=True)
    print(
        f"[voice/diag] connect() returned in {loop.time()-t0:.2f}s — "
        f"is_connected={vc.is_connected()} "
        f"endpoint={getattr(vc, 'endpoint', '?')}",
        flush=True,
    )

    # Pycord's `await voice_channel.connect()` USUALLY returns after
    # SESSION_DESCRIPTION arrives, but with reconnect=True it can return
    # earlier on the first attempt. Poll vc.is_connected() to be safe.
    handshake_t0 = loop.time()
    for i in range(100):  # 10s
        if vc.is_connected():
            print(f"[voice/diag] handshake complete after {loop.time()-handshake_t0:.2f}s", flush=True)
            break
        await asyncio.sleep(0.1)
    if not vc.is_connected():
        elapsed = loop.time() - handshake_t0
        print(
            f"[voice/diag] handshake TIMED OUT after {elapsed:.1f}s — "
            f"vc.endpoint={getattr(vc, 'endpoint', '?')} "
            f"vc._connected.is_set={vc._connected.is_set() if hasattr(vc, '_connected') else '?'} "
            f"bot.voice_clients={len(getattr(bot_client, 'voice_clients', []) or [])}",
            flush=True,
        )
        try:
            ...
            pass
        return {"ok": False, "error": "voice handshake did not complete within 10s — likely network egress / UDP block"}
    source = StreamingPCMAudio()
    done = asyncio.Event()
    ...
    # Kick off synthesizer in a background thread BEFORE vc.play() so the
    # first frames are queued by the time the audio worker starts pulling.
    synth_thread = _threading.Thread(
        target=_stream_synthesis_to_source,
        args=(text, backend_name, source),
        name=f"tts_synth_{voice_channel_id}",
        daemon=True,
    )
    synth_thread.start()
...
async def speak_voice(req: SpeakVoiceRequest):
    ...
    try:
        ...
        # Log full traceback so voice failures surface in the container log
        # — the user only sees the {"error": str(e)} JSON otherwise.
        import traceback
        print(f"[voice] /speak_voice failed for vc={req.voice_channel_id}: {e}", flush=True)
        traceback.print_exc()
        return {"ok": False, "error": str(e)}
