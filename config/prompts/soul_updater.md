...
Your task is to rewrite `/state/soul/SOUL.md` based on accumulated context.
...
1. Read the current `state/soul/SOUL.md` to understand the existing persona.
2. Read `config/identity/USER.md` to understand who is being served.
3. Note the current identity, voice, and guidance already encoded — you will improve on this, not duplicate it.

## Phase 2 — Gather

1. Read `state/soul/MEMORY.md` for accumulated long-term facts.
2. List and read any files in `state/memory/` for recent session insights.
...
3. Once the draft fits, write the new SOUL.md using `file_write` with path `state/soul/SOUL.md`.
