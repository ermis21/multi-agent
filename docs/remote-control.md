...
```
  ...
  │  (your dev)   │    │  (FastAPI WS server)  │
  └──────────────┘    │  phoebe-discord       │
                      └──────────────────────┘
```

- **Pi** runs on any machine (your dev box, server, etc.)
- **rpc-proxy** bridges Pi's stdin/stdout JSONL to WebSocket
- **Phoebe bridge** accepts WebSocket connections, multiplexes multiple Pi instances, renders events to Discord
- **Discord** provides the UI — you see Pi's work in real-time and can steer/abort/bash
...
remote:
  enabled: true
  ws_port: 8090           # same port as phoebe-api (FastAPI handles WS upgrade)
  ws_host: your-phoebe-host-ip    # set to your Phoebe host's IP or hostname
```

### 2. Expose the API port

Port 8090 is already exposed by default. Ensure it's accessible from your Pi machine
(your firewall, router, or Tailscale tailnet must allow incoming connections).

### 3. Connect Pi from your machine
...
Test the bridge against a local Pi instance (no remote machine needed):
